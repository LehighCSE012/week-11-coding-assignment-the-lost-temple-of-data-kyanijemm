"""Weekly Dungeon Code Assgignment - Kyani's Adventure Part 11"""
...
def load_artifact_data(excel_filepath):
    ...
    df_excel = pd.read_excel(excel_filepath,sheet_name='Main Chamber',skiprows=3)
    return df_excel
...
def load_location_notes(tsv_filepath):
    ...
    # Replace 'pass' with your code
    df_tsv = pd.read_csv(tsv_filepath, sep='\t')
    return df_tsv
    # return the resulting DataFrame
...
def extract_journal_dates(journal_text):
    ...
    found_dates = re.findall(pattern,journal_text)
    return found_dates
...
def extract_secret_codes(journal_text):
    ...
    # Replace 'pass' with your code
    pattern = r"AZMAR-\d{3}"
    found_codes = re.findall(pattern,journal_text)
    return found_codes
